...
public class RailWagonTankModel extends RailWagon0Model {
	...
	public RailWagonTankModel(double dx, double dy, double dz, double dxFront, double dyFront, double dzFront,
			double dxRear, double dyRear, double dzRear, double dxRoof, double dyRoof, double dzRoof,
			double rearOverhang, double frontOverhang, double wheelRadius, double wheelWidth, int wheelRays) {
		super(dx, dy, dz, dxFront, dyFront, dzFront, dxRear, dyRear, dzRear, dxRoof, dyRoof, dzRoof, rearOverhang,
				frontOverhang, wheelRadius, wheelWidth, wheelRays);
	}
	...
	protected void buildTankWagonSideTexture(double x, double y) {
		...
		double dxWidth=Math.PI*dxRoof/wagonRays;
		for (int i = 0; i < wagonRays; i++) {
			addTRect(x, y, dxWidth, dyRoof);
			x += dxWidth;
		}
	}

	protected void buildTankWagonBackTexture(double x, double y) {
		double radius=dxRoof*0.5;
		buildWheelTextures(x, y, radius, radius, radius, wagonRays,+1, 0);
	}

	protected void buildTankWagonFrontTexture(double x, double y) {
		double radius=dxRoof*0.5;
		buildWheelTextures(x, y, radius, radius, radius, wagonRays,+1, 0);
	}
