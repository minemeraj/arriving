...
public class RailWagonCoachModel extends RailWagonBulkModel {
	...
	public RailWagonCoachModel(double dx, double dy, double dz, double dxFront, double dyFront, double dzFront,
	...
	protected int buildWagonFaces(int counter) {
		...
		for (int i = 0; i < rnumx - 1; i++) {
			...
			for (int j = 0; j < rnumy - 1; j++) {
				...
				faces[counter++] = buildFace(Renderer3D.CAR_TOP, roof[i][j], roof[i + 1][j], roof[i + 1][j + 1],
						roof[i][j + 1], tBo[0]);
		...
		// back roof
		faces[counter++] = buildFace(Renderer3D.CAR_BACK, roof[2][1], roof[1][1], roof[0][1], tBo[0]);

		// front roof
		faces[counter++] = buildFace(Renderer3D.CAR_FRONT, roof[0][1], roof[1][1], roof[2][1], tBo[0]);
