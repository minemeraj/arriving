...
public class OldRoadEditor extends JFrame implements MenuListener,ActionListener,MouseListener,MouseWheelListener,PropertyChangeListener,MouseMotionListener,KeyListener, ItemListener{
	...
	private void displayRoad(Graphics2D bufGraphics,BufferedImage buf) {
		...
		for(int j=0;j<NY-1;j++){
			...
			drawPolygon(roadData[j],roadData[j+1],j,bufGraphics,buf);
				...
				if(roadData[j][i].isSelected()){
					...
					if(i<NX-1 && j<NY-1){
						...
						bufGraphics.drawLine(
								convertX(roadData[j][i].x),
								convertY(roadData[j][i].y),
								convertX(roadData[j][i+1].x),
								convertY(roadData[j][i+1].y)
						);

						bufGraphics.drawLine(
								convertX(roadData[j][i+1].x),
								convertY(roadData[j][i+1].y),
								convertX(roadData[j+1][i+1].x),
								convertY(roadData[j+1][i+1].y)
						);
						
						bufGraphics.drawLine(
								convertX(roadData[j+1][i+1].x),
								convertY(roadData[j+1][i+1].y),
								convertX(roadData[j+1][i].x),
								convertY(roadData[j+1][i].y)
						);
	...
	private void drawPolygon(Point4D[] is, Point4D[] is2,int j,Graphics2D bufGraphics,BufferedImage buf) {
		...
		for(int i=0;i<is.length-1;i++){
			...
			Polygon3D p3dr=new Polygon3D(4,cxr,cyr,czr);
	...
	public static void main(String[] args) {
		...
		OldRoadEditor re=new OldRoadEditor();
		re.initialize();
	}
		...
		if(regm.getReturnValue()!=null){
			...
			for(int j=0;j<NY;j++)
				...
				for(int i=0;i<NX;i++){
					...
					roadData[j][i].x=deltax*i;
					roadData[j][i].y=deltay*j;
					roadData[j][i].z=0;
					roadData[j][i].setHexColor("888888");
					roadData[j][i].setIndex(0);
