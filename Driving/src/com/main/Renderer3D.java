...
public abstract class Renderer3D extends DrivingFrame implements AbstractRenderer3D {
	...
	public static final int[] faceIndexes = { CAR_BACK, CAR_TOP, CAR_LEFT, CAR_RIGHT, CAR_FRONT, CAR_BOTTOM };

	public static final String[] faceDesc = { "Ba", "To", "Le", "Ri", "Fr", "Bo" };
	...
	/**
	 *
	 * DECOMPOSE PROJECTED TRIANGLE USING EDGE WALKING AND PERSPECTIVE CORRECT
	 * MAPPING
	 *
	 * @param p3d
	 * @param color
	...
	public void decomposeTriangleIntoZBufferEdgeWalking(Polygon3D p3d, int rgbColor, Texture texture, ZBuffer zb,
		...
		if (texture != null) {
			...
			mip_map_level = (int) (0.5
					* Math.log(
							bc.getRealTriangleArea() / BarycentricCoordinates.getTriangleArea(x0, y0, x1, y1, x2, y2))
					/ Math.log(2));
			...
			/*
			 * points[0].setTexurePositions(ZBuffer.
			 * pickTexturePositionPCoordinates(texture,points[0].p_x,points[0].
			 * p_y,points[0].p_z,xDirection,yDirection,origin,deltaX,deltaY));
			 * points[1].setTexurePositions(ZBuffer.
			 * pickTexturePositionPCoordinates(texture,points[1].p_x,points[1].
			 * p_y,points[1].p_z,xDirection,yDirection,origin,deltaX,deltaY));
			 * points[2].setTexurePositions(ZBuffer.
			 * pickTexturePositionPCoordinates(texture,points[2].p_x,points[2].
			 * p_y,points[2].p_z,xDirection,yDirection,origin,deltaX,deltaY));
			 */
		}
		...
		for (int j = j0; j < j1; j++) {
			...
			Point3D pstart = new Point3D(middlex, j, intersects.p_z, intersects.p_x, intersects.p_y, intersects.p_z,
					intersects.texture_x, intersects.texture_y);
			Point3D pend = new Point3D(middlex2, j, intersecte.p_z, intersecte.p_x, intersecte.p_y, intersecte.p_z,
					intersecte.texture_x, intersecte.texture_y);
			...
			for (int i = i0; i < end; i++) {
				...
				// rgbColor=ZBuffer.pickRGBColorFromTexture(texture,xi,yi,zi,xDirection,yDirection,origin,deltaX,
				// deltaY,bc);
				...
				zb.set(xi, yi, zi, yi, calculateShadowColor(xi, yi, zi, cosin, rgbColor, p3d.isWaterPolygon()), level,
						tot, hashCode);
		...
		for (int j = j0; j < j1; j++) {
			...
			Point3D pstart = new Point3D(middlex, j, intersects.p_z, intersects.p_x, intersects.p_y, intersects.p_z,
					intersects.texture_x, intersects.texture_y);
			Point3D pend = new Point3D(middlex2, j, insersecte.p_z, insersecte.p_x, insersecte.p_y, insersecte.p_z,
					insersecte.texture_x, insersecte.texture_y);
			...
			for (int i = i0; i < end; i++) {
				...
				if (texture != null)
					// rgbColor=ZBuffer.pickRGBColorFromTexture(texture,xi,yi,zi,xDirection,yDirection,origin,
					// deltaX,deltaY,bc);
					rgbColor = texture.getRGBMip((int) texture_x, (int) texture_y, mip_map_level);
				if (rgbColor == greenRgb)
					continue;
	...
	private int calculateShadowColor(double xi, double yi, double zi, double cosin, int argbs, boolean isWater) {
		...
		// double
		// factor=(lightIntensity*(0.25+0.5*Math.exp(-yi*0.001)+0.25*cosin));
	...
	public void decomposeClippedPolygonIntoZBuffer(Polygon3D p3d, Color color, Texture texture, ZBuffer zbuffer,
		...
		for (int i = 0; i < triangles.length; i++) {
			...
			for (int j = 0; j < clippedTriangles.length; j++) {
				...
				decomposeTriangleIntoZBufferEdgeWalking(clippedTriangles[j], color.getRGB(), texture, zbuffer,
						xDirection, yDirection, origin, deltaX, deltaY, bc, hashCode);
	...
	void decomposeCubicMesh(CubicMesh cm, Texture texture, ZBuffer zBuffer) {
		...
		for (int i = 0; i < polSize; i++) {
			...
			decomposeCubiMeshPolygon(polRotate, xVersor, yVersor, zVersor, zMinusVersor, cm, point000, point011,
					point001, face, col, texture, zBuffer);

		}

	}

	void decomposeCubiMeshPolygon(Polygon3D polRotate, Point3D xVersor, Point3D yVersor, Point3D zVersor,
			Point3D zMinusVersor, CubicMesh cm, Point3D point000, Point3D point011, Point3D point001, int face,
			Color col, Texture texture, ZBuffer zBuffer) {
		...
		decomposeClippedPolygonIntoZBuffer(polRotate, col, texture, zBuffer, xDirection, yDirection, rotateOrigin,
				deltaTexture + deltaWidth, deltaHeight, hashCode);
	}
	...
	private void decomposePointIntoZBuffer(double xs, double ys, double zs, double ds, int rgbColor, ZBuffer zbuffer) {

		if (ys <= SCREEN_DISTANCE)
			return;
	...
	Polygon3D buildTransformedPolygon3DWithoutTexture(LineData ld, double[] xpoints, double[] ypoints, double[] zpoints,
			int level) {
	...
	Polygon3D buildTransformedPolygonProjection(LineData ld, double[] xpoints, double[] ypoints, double[] zpoints,
			int level) {
	...
	public static int getFace(LineData ld, double[] xpoints, double[] ypoints, double[] zpoints) {
		...
		Point3D p0 = new Point3D(xpoints[ld.getIndex((n + 0 - 1) % n)], ypoints[ld.getIndex((n + 0 - 1) % n)],
				zpoints[ld.getIndex((n + 0 - 1) % n)]);
		Point3D p1 = new Point3D(xpoints[ld.getIndex(0)], ypoints[ld.getIndex(0)], zpoints[ld.getIndex(0)]);
		Point3D p2 = new Point3D(xpoints[ld.getIndex((1 + 0) % n)], ypoints[ld.getIndex((1 + 0) % n)],
				zpoints[ld.getIndex((1 + 0) % n)]);
