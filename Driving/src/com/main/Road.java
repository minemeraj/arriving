...
public class Road extends Shader {
	...
	protected CubicMesh steeringWheel = null;
	protected Texture steeringWheelTexture;
	...
	private void loadObjectsFromFile(File file) {
		...
		for (int i = 0; i < vdrawObjects.size(); i++) {
			drawObjects[i] = vdrawObjects.get(i);
		}
	...
	@Override
	public void buildRectanglePolygons(ArrayList<Polygon3D> polygons, double x, double y, double z, double dx,
			double dy, double dz) {
	 ...
	 * SCALING VALUES:
	 *
	 * L=2.6 METRES =160 px REAL TIME dt=0.05 sec.
	 *
	 */
	...
	public void calculateShadowMap() {
		...
		for (int ii = 0; ii < 2; ii++) {
			...
			for (int j = 0; j < size; j++) {
				...
				LineData ld = meshes[index].polygonData.get(j);

				Polygon3D p3D = buildLightTransformedPolygon3D(ld, ii);

				decomposeClippedPolygonIntoZBuffer(p3D, ZBuffer.fromHexToColor(p3D.getHexColor()),
						EditorData.worldTextures[p3D.getIndex()], lightZbuffer, -1);
		...
		for (int ii = 0; ii < drawObjects.length; ii++) {
			...
			for (int i = 0; i < polSize; i++) {
				...
				decomposeCubiMeshPolygon(polRotate, xVersor, yVersor, zVersor, zMinusVersor, cm, point000, point011,
						point001, face, col, texture, lightZbuffer);
	...
	private void buildCar(double directionAngle) {
		...
		/*
		 * double dx = WIDTH / 2 - CAR_WIDTH / 2 - XFOCUS; double dy = y_edge;
		 * double dz = -YFOCUS;
		 */
		...
		for (int i = 0; i < polSize; i++) {
			...
			decomposeCubiMeshPolygon(polRotate, xVersor, yVersor, zVersor, zMinusVersor, cm, cm.point000, cm.point011,
					cm.point001, face, col, carTexture, roadZbuffer);
	...
	private Point3D rotoTransform(double[][] rot,
			...
			Point3D p, double dx, double dy, double dz) {
	...
	void drawRoad(BufferedImage buf) {
		...
		for (int j = 0; j < pSize; j++) {
			...
			decomposeClippedPolygonIntoZBuffer(p3D, ZBuffer.fromHexToColor(p3D.getHexColor()),
					EditorData.worldTextures[p3D.getIndex()], roadZbuffer, hashCode);
	...
	private void drawCarInside(ZBuffer roadZbuff) {
		if (steeringWheel != null) {

			CubicMesh mesh = steeringWheel.clone();
			...
			decomposeCubicMesh(mesh, steeringWheelTexture, roadZbuff);
		}
	}
	...
	private void calculateDrawingData(PolygonMesh mesh, boolean isTerrain) {
		...
		for (int j = 0; j < size; j++) {
			...
			Polygon3D p3DProjection = buildTransformedPolygonProjection(ld, mesh.xpoints, mesh.ypoints, mesh.zpoints,
					mesh.getLevel());
			...
			if (autocars != null) {
				...
				for (int i = 0; i < autocars.length; i++) {
					...
					if (p3DProjection.contains(transformedCenter.x, transformedCenter.y)) {
						...
						Polygon3D p3D = buildTransformedPolygon3DWithoutTexture(ld, mesh.xpoints, mesh.ypoints,
								mesh.zpoints, mesh.getLevel());

						Polygon3D realP3D = LineData.buildPolygon(ld, mesh.xpoints, mesh.ypoints, mesh.zpoints);

						int posz = (int) interpolate(transformedCenter.x, transformedCenter.y, p3D);

						autocars[i].center.z = autocars[i].car_height * 0.5 + (posz);
	...
	private void drawSPLines(ArrayList<SPLine> splines, Rectangle totalVisibleFieldBounds, ZBuffer roadZbuffer) {
		...
		for (int i = 0; i < szMesh; i++) {
			...
			for (int j = 0; j < sizeMeshes; j++) {
				...
				for (int k = 0; k < size; k++) {
					...
					Polygon3D p3DBase = buildTransformedPolygon3DWithoutTexture(ld, mesh.xpoints, mesh.ypoints,
							mesh.zpoints, mesh.getLevel());
				...
				for (int k = 0; k < size; k++) {
					...
					Polygon3D p3D = buildTransformedPolygon3D(ld, mesh.xpoints, mesh.ypoints, mesh.zpoints,
							mesh.getLevel());

					decomposeClippedPolygonIntoZBuffer(p3D, ZBuffer.fromHexToColor(p3D.getHexColor()),
							EditorData.splinesTextures[ld.getTexture_index()], roadZbuffer, hashCode);
	...
	public void calculateStencilBuffer() {
		...
		if (carShadowVolume[SELECTED_CAR] == null || carShadowVolume[SELECTED_CAR].allPolygons == null
				|| rearAngle != 0) {
			// do nothing
	...
	public static Polygon3D[] buildAdditionalRoadPolygons(Polygon3D p3d) {
		...
		for (int i = 0; i < p3d.npoints; i++) {
			...
			pols[i].addPoint(p3d.xpoints[i], p3d.ypoints[i], p3d.zpoints[i] - ROAD_THICKNESS, p3d.xtpoints[i],
					p3d.ytpoints[i]);
			pols[i].addPoint(p3d.xpoints[(i + 1) % p3d.npoints], p3d.ypoints[(i + 1) % p3d.npoints],
					p3d.zpoints[(i + 1) % p3d.npoints] - ROAD_THICKNESS, p3d.xtpoints[(i + 1) % p3d.npoints],
					p3d.ytpoints[(i + 1) % p3d.npoints]);
			pols[i].addPoint(p3d.xpoints[(i + 1) % p3d.npoints], p3d.ypoints[(i + 1) % p3d.npoints],
					p3d.zpoints[(i + 1) % p3d.npoints], p3d.xtpoints[i], p3d.ytpoints[(i + 1) % p3d.npoints]);
	...
	/**
	 * alfa = scale factor: 2PI:(tetam*2.0)=dw:(alfa*WIDTH)
	 * i:WIDTH=teta:(tetam*2.0) 0<i<WIDTH, i screen x point i_set:dw=teta:2PI
	 * i_set background x point
	 *
	 * texture and screen with y axis downward oriented:
	 *
	 * j=0 -> j_set= dh- (YFOCUS*alfa) j=YFOCUS -> j_set= dh
	 */
	...
	private void drawAutocars(Autocar[] autocars, Rectangle totalVisibleFieldBounds, ZBuffer roadZbuffer) {
		...
		for (int i = 0; i < autocars.length; i++) {
			...
			cm.rotateZ(autocar.center.x, autocar.center.y, cosRo, sinRo);
	...
	private void rotoTransform(double[][] aRotation, CubicMesh cm, double dx, double dy, double dz) {
	...
	boolean checkIsWayFree(int new_posx, int new_posy, double newViewDirection, int autocar_index) {
		...
		if (autocars != null) {
			...
			for (int l = 0; l < autocars.length; l++) {
				...
				Polygon polyCar = autocars[l].buildCarBox((int) autocars[l].center.x, (int) autocars[l].center.y,
						(int) autocars[l].center.z);
	...
	private Point3D getIntersection(Polygon polyCar, Polygon carBorder) {
		...
		for (int i = 0; i < polyCar.npoints; i++) {
			...
			Point3D poly1 = new Point3D(polyCar.xpoints[(i + 1) % polyCar.npoints],
					polyCar.ypoints[(i + 1) % polyCar.npoints], 0);

			for (int j = 0; j < carBorder.npoints; j++) {

				Point3D carP0 = new Point3D(carBorder.xpoints[j], carBorder.ypoints[j], 0);
				Point3D carP1 = new Point3D(carBorder.xpoints[(j + 1) % carBorder.npoints],
						carBorder.ypoints[(j + 1) % carBorder.npoints], 0);
	...
	private static void generateDefault0() {
		...
		for (int j = 0; j < ny; j++) {
			for (int i = 0; i < nx; i++) {
				...
				System.out.println("f=T2 " + "C00FF00 " + n0 + "/0 " + n1 + "/1 " + n2 + "/2 " + n3 + "/3");
